'use strict';
/**
 * Offline finalize for wedged shell: node scripts/_offline-write-pres-lga.js
 * Reads agent-tools dump, writes the three required outputs.
 */
const fs = require('fs');
...
function loadDump(file) {
  ...
  // Fix string-aware JSON scan (agent-tools dumps may append markdown).
  let depth = 0;
  let end = -1;
  let inString = false;
  let escape = false;
...
function main() {
  const dump = loadDump(DUMP);
  fs.mkdirSync(RAW_DIR, { recursive: true });
  fs.mkdirSync(DATA_DIR, { recursive: true });

  // Compact fetch metadata (units omitted — huge)
  const { units: _u, ...metaOnly } = dump;
  fs.writeFileSync(
    path.join(RAW_DIR, 'stears-pres-lga-fetch.json'),
    JSON.stringify({ ...metaOnly, unitsOmitted: true, totalUnitKeys: Object.keys(dump.units || {}).length }) + '\n'
  );
  ...
  const statePayload = JSON.parse(fs.readFileSync(path.join(DATA_DIR, 'presidential-2023-states.json'), 'utf8'));
  const mismatches = [];
  const matched = [];
  ...
  const lagos = Object.keys(units).filter((k) => k.startsWith('Lagos::')).length;
  const marker = path.join(RAW_DIR, 'stears-pres-lga-offline-done.txt');
  fs.writeFileSync(
    marker,
    JSON.stringify(
      {
        ...
        lagosLgas: lagos,
        unitKeys: Object.keys(units).length,
      },
      null,
      2
    )
